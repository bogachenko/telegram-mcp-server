...
function renderHealth(){
  ...
  body.innerHTML=problemItems.map(s=>{
    ...
    const action=s.enabled?'<button class="secondary" onclick="syncSource(\''+js(s.id)+'\',0)">Проверить</button>':'<span class="small">Отключён</span>';
    return '<tr><td><span class="pill">'+esc(healthStatusLabel(status))+'</span></td><td><b>'+esc(s.id)+'</b>'+title+'</td><td>'+esc(s.type)+'</td><td>'+esc(s.error_count||0)+'</td><td>'+paused+'</td><td style="max-width:420px">'+lastError+'</td><td><div class="actions">'+action+'</div></td></tr>';
...
async function syncSource(id,backfill){try{const data=await api('/admin/api/sync',{method:'POST',body:JSON.stringify({source_id:id,backfill:backfill,limit:200})});show('Sync: '+JSON.stringify(data.results||data));await loadAll();if(document.getElementById('admin-page-messages')?.style.display!=='none')await loadRecent()}catch(err){show(err.message,true)}}
async function syncAll(){try{const data=await api('/admin/api/sync',{method:'POST',body:JSON.stringify({limit:200})});show('Sync all: '+JSON.stringify(data.results||data));await loadAll();if(document.getElementById('admin-page-messages')?.style.display!=='none')await loadRecent()}catch(err){show(err.message,true)}}
